...
dbutils.widgets.text("ncontrol_parts", "2")
dbutils.widgets.text("ntarget_parts", "2")
dbutils.widgets.text("seed", "42")

dbutils.widgets.text("straps", "1000")
dbutils.widgets.text("bootstrap_stat", "median")

dbutils.widgets.text(f"variants_cache_path", "/Volumes/sports_iq/abatar_param/cache/variant_cache.json")
...
boot_stat = dbutils.widgets.get("bootstrap_stat")

variants_cache_path = dbutils.widgets.get("variants_cache_path")

# COMMAND ----------

with open(variants_cache_path, 'r') as file:
    variant_data = json.load(file)
    control = variant_data['control']
    target = variant_data['target']

# COMMAND ----------

control_df = pd.Series(control)
control_df.info(show_counts=True)

# COMMAND ----------

target_df = pd.Series(target)
target_df.info(show_counts=True)

# COMMAND ----------

pd.concat([pd.DataFrame({'control':control}).describe().round(), pd.DataFrame({'target':target}).describe().round()], axis=1)
...
def permutation_test_spark(control
    ...
    Parameters:
        control : The control group values.
        target : The target group values.
    ...
    null_dist = np.array(rdd.map(permutation_stat_rdd).collect())
...
permu_report = pt._generate_result_message(pvalue, obs_stat)[:-250]
...
boot_report = bt._generate_result_message(uplift_percent_est, uplift_diff_est)
...
dbutils.notebook.exit(json.dumps({'obs_stat':obs_stat.tolist()
                                  ...
                                  , 'reject_H0':reject_H0
                                  , 'permutation_report':permu_report
